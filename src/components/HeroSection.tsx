...
// 風格條 6 項：中文（粗體）+ 英文；圖示統一用模板 logo /brand-logos/4.svg
const STYLE_TAGS = [
  { zh: '現代風', en: 'Modern' },
  { zh: '輕奢風', en: 'Modern Luxury' },
  { zh: '北歐風', en: 'Scandinavian' },
  { zh: '工業風', en: 'Industrial' },
  { zh: '美式風', en: 'American' },
  { zh: '鄉村風', en: 'Country' },
];

function BrandTag({ zh, en }: { zh: string; en: string }) {
  return (
    <a href="#" title={zh} className="group shrink-0 flex items-center gap-3">
      <img
        src="/brand-logos/icon.svg"
        alt=""
        className="h-[34px] w-[34px] opacity-60 group-hover:opacity-100 transition-opacity shrink-0"
      />
  ...
  return (
    <div className="relative z-10">
      ...
      <section className="relative w-full overflow-hidden" style={{ height: '958px' }}>
        ...
        >
          <div className="lg:absolute lg:left-[30px] lg:top-[244px] flex flex-col justify-center h-full lg:h-auto lg:block">
      ...
      {/* 風格條：6 個靜態項（不跑馬燈），圖示統一用模板 logo /brand-logos/4.svg + 中文/英文 */}
      <div
      ...
      >
        {STYLE_TAGS.map((t, i) => (
          <BrandTag key={i} zh={t.zh} en={t.en} />
        ))}
      </div>
