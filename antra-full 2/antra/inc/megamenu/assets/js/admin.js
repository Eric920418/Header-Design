( function($) {
	var Antra_Modal_Menu_Item = Backbone.View.extend({

		initialize: function( data ) {
			this.model = new Backbone.Model( data );
			this.template = '<span class="antra-edit-menu"><i class="fa fa-pencil"></i> Mega</span>';
		...
		_openModalHandler: function() {
			window.antra_menu_modal = new Antra_Modal( this.model );
		}

	});

	// Backbone Modal View
	var Antra_Modal = Backbone.View.extend({
		...
		initialize: function( model ) {
			...
			this.events = {
				...
				'click .antra-modal-overlay' : '_closeHandler',
		...
		render: function() {
			var that = this;
			this.template = $( '#tpl-antra-menu-item-modal' ).html();
			...
			data.action = 'antra_load_menu_data';
			data.nonce = antra_memgamnu_params.nonces.load_menu_data;
			...
			if ( this.model.get( 'is_loading' ) === true ) {
				$.ajax({
					type: 'POST',
					url: antra_memgamnu_params.ajaxurl,
					data: data,
					beforeSend: function() {
		...
		_submitEditMenuForm: function( e ) {
			...
			data.push({
				name: 'action',
				value: 'antra_update_menu_item_data'
			});
			...
			$.ajax({
				...
				data: data,
				beforeSend: function() {
					form.find( '.antra-modal-footer button' ).attr( 'disabled', true );
					form.find( 'button[type="submit"]' ).prepend( '<i class="fa fa-spin fa-spinner"></i> ' );
				}
			}).always( function() {
				form.find( '.antra-modal-footer button' ).attr( 'disabled', false );
			} ).done( function( res ) {
				if ( res.status === true ) {
					that.model.destroy();
				}
	...
	$( document ).on( 'mouseenter', '#menu-to-edit .menu-item-handle', function(e) {
		...
		menu_title.append( new Antra_Modal_Menu_Item({
			menu_id: menu_id,
			is_loading: true,
			istop:istop?1:0
		}).render().el );
		...
		var menu_item = $( this );
		var menu_title = menu_item.find( '.item-title' );
		menu_title.find( '.antra-edit-menu' ).remove();
	} );
} )(jQuery);
