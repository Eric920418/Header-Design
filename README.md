...
- 2026-08-25 Footer 的回到頂端按鈕移出 Footer 裁切容器，改為全站固定於視窗右下角；手機／桌機分別保留 84／88px 右距以避開右側快捷列，任何捲動位置都持續可見。點擊後平滑回到頁首，`prefers-reduced-motion` 下改為立即跳回頂端。
- 2026-08-25 首頁 Hero 的 `Trusted Design Partner` 膠囊維持 30px 高、24px 圓角與原灰色色相，只將邊框透明度由 18% 提升至 55%，解決深色圖片上外框幾乎不可見的問題，不改金點、文字、間距或動畫。
- 首頁 Hero 文字依 Home 06 使用 `slideInLeft／fadeIn／fadeInUp`，Hero 圖片依 Home 01 使用雙層向下揭幕；其他首頁小標、主標與 CTA 才沿用各自模板的 `opalMove*／opalScaleUp`。CTA 保留雷達水波、箭頭旋轉與 hover 填金；`prefers-reduced-motion` 會關閉進場、輪播、翻牌與水波，但所有內容立即可見。共用 Header／Footer 調整會套用全站，其他改動僅影響首頁。
