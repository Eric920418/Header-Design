...
- **提案預覽密碼鎖（2026-09-03）**：全站頁面由 Nitro 伺服器中介層保護，未驗證訪客會導向 `/preview-access`；預設密碼改為無空格格式。密碼經伺服器端 SHA-256 定長比對後，簽發包含絕對到期時間與 HMAC 簽章的 1 小時 HttpOnly Cookie。伺服器會拒絕過期或遭竄改的權杖，前端也會在到期、分頁重新取得焦點或切頁時立即導回密碼頁，不再只依賴瀏覽器刪除 Cookie。正式環境可用 `NUXT_PREVIEW_PASSWORD` 覆蓋；靜態圖片與 Nuxt 執行資源維持可載入，API 未授權時回傳完整 401 錯誤。
