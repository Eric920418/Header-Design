...
- **移動中「卡間縫隙」防治（兩層）**：Embla `loop` 靠每張卡的 `translate3d(迴圈總長=10×378=3780)` 環繞，且軌道以**小數 transform** 位移。
  1. **次像素髮絲縫**：`<article>` 用 `w-[calc(100%+1px)]`（比卡槽 `.group` 寬 1px、右緣蓋住與下一張的接縫），避免小數位移時卡間透出背景色細線。卡槽仍 378，**不影響 Embla 量測/迴圈總長**。
  2. **展開接縫縫隙**：`mouseleave` **延遲 560ms 才恢復**自動輪播（`resumeTimer`），確保 hover 卡片的 500ms 寬度動畫**完全收回、所有卡等寬後**輪播才移動；否則卡片變寬期間迴圈總長是舊值 3780、環繞接縫露縫。（自動化隱藏分頁 rAF 不跑、無法重現移動，需在可見瀏覽器驗證。）
...
- **三欄（`grid-cols-3 gap-[30px]`，各 450、`min-h-[673px]`）**：
  - **欄1 SAKURA 深色圖卡**：暗色廚電圖 + 漸層遮罩；標題改用**官方白色字標** `public/sakura-kitchen-logo.png`（來源 `IMG_1188.PNG`，純白 RGB 255、555×90、粗體「SAKURA」單字，比例 6.17）取代原 `<h3>SAKURA 廚電</h3>`（保留為 `sr-only`）；alt=`SAKURA`；`img h-[54px] w-auto self-start`（**`self-start` 必要**：父層 `flex flex-col` 預設 `align-items:stretch` 會把 img 橫向撐滿到卡寬 370→壓扁,故用 `self-start` 回自然寬、比例 6.17；**h-28→寬 173，內容區 370（與 svago/teka 標題份量均衡）**。原生 555px 寬 > 顯示寬，恆為縮小顯示、不糊）。下接三產品線金勾清單（廚房商品／淨水器／熱水器，`Check` 金圓）。與 SVAGO/TEKA 白卡的 logo 圖手法一致（白卡用黑版 SVG、深卡用白版 PNG）。**SVAGO/TEKA logo（白卡）**：`img w-auto` + **各品牌自訂 `logoH`**（`BRANDS[].logoH`：svago 38 / teka 48）——因兩 SVG 內部留白不同（canvas 量：svago 字形填 99.6%、teka 僅 64.3%），同高會不等大，故按「可見字形」對齊。logo 放進**固定 `h-[48px] flex items-center` 的列並垂直置中** → 兩卡 logo 區等高，故 logoH 差(38/48)**不會推歪**下方描述位置與 `flex-1` 產品圖高度（先前差 10px：描述不同高、圖片不等高，即此列高度差連鎖所致）。`svago.svg` 由使用者白色 PNG（`IMG_1186.PNG`，去掉「JOY OF COOKING」副標的簡潔版）經 **potrace 向量化 + 填黑** 而成（白卡可見、任意縮放銳利）。
