...
- **三欄（`grid-cols-3 gap-[30px]`，各 450、`min-h-[673px]`）**：
  - **欄1 SAKURA 深色圖卡**：暗色廚電圖 + 漸層遮罩；標題改用**官方白色字標** `public/sakura-kitchen-logo.png`（來源 `首頁用圖 2/Logo/2023 SAKURA KITCHEN…LOGO_3.png`，純白 RGB 255）取代原 `<h3>SAKURA 廚電</h3>`（保留為 `sr-only`）；`img h-[54px] w-auto self-start`（**`self-start` 必要**：父層 `flex flex-col` 預設 `align-items:stretch` 會把 img 橫向撐滿到卡寬 370→壓扁,故用 `self-start` 回自然寬、比例 6.53；**h-56→寬 366，貼滿內容區 370（右留 4px，為不溢出上限）**。原檔 3508px 超高解析，放大不糊）。下接三產品線金勾清單（廚房商品／淨水器／熱水器，`Check` 金圓）。與 SVAGO/TEKA 白卡的 logo 圖手法一致（白卡用黑版 SVG、深卡用白版 PNG）。**SVAGO/TEKA logo（白卡）**：`img w-auto` + **各品牌自訂 `logoH`**（`BRANDS[].logoH`：svago 38 / teka 48）——因兩 SVG 內部留白不同（canvas 量：svago 字形填 99.6%、teka 僅 64.3%），同高會不等大，故按「可見字形」對齊。`svago.svg` 由使用者白色 PNG（`IMG_1186.PNG`，去掉「JOY OF COOKING」副標的簡潔版）經 **potrace 向量化 + 填黑** 而成（白卡可見、任意縮放銳利）。
