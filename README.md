...
- **內容與素材**：01–06 英文 title／excerpt 完整保留模板 dummy-data；`service-{1..6}.jpg`、`h6-bg-2.jpg` 與兩個 deco SVG 已從 Page 6 原始素材本地化到 `public/services/`，不再依賴 demo 外連；破圖仍 fallback `/kitchen-styles/elegant.jpg`。使用者指定第 04 張卡將 `Outdoor & Landscape Design` h3 替換為 `public/services/sakura-logo-white.png`；因原圖純白、caption 為白底，沿用原 h3 的深色視覺（`brightness(0)` + 89% opacity）以保證可見。第 04 張卡主圖片同步改用使用者提供的抽油煙機視覺 `public/services/sakura-product.png`（977×945）；卡片資料與輪播不變。
...
- **Browser checks**：1512、1024、390 三種寬度均無水平溢出、素材 natural size 正確（服務圖 `1410×1018`）、無 console error。第 02 卡 SVago logo 在 1512px 為 `125.32×35`、390px 為 `107.41×30`；其新主圖 natural size `1300×945`，在 1512px 為 `430×310`、390px 為 `340×250`。第 03 卡 TEKA SVG 在 1512px 為 `119×35`、390px 為 `102×30`；其主圖 natural size `977×945`，在 1512px 為 `430×310`、390px 為 `340×250`。第 04 卡 SAKURA logo 在 1512px 為 `210×34.05`、390px 為 `170×27.56`；抽油煙機主圖在 1512px 為 `430×310`、390px 為 `340×250`。三張主圖均以 `object-cover` 正確填滿卡片。
- **品牌標題素材**：第 02 卡改用使用者提供的 `public/services/svago-logo-white.png`（SVago，`111×31`）取代原英文標題，主圖同步換成 `public/services/svago-product.png`（`1300×945`）；第 03 卡改用 `public/services/teka-logo-white.svg`（TEKA 向量 logo，viewBox `170×50`）與 `public/services/teka-product.png`（`977×945`）；第 04 卡維持 SAKURA logo 與抽油煙機主圖。三張來源皆為白色透明圖，因此在白底 caption 中統一轉為原 h3 的深色。SVago／TEKA 使用 mobile `30px`／tablet `32px`／desktop `35px` 的固定高度和 `width:auto`，**保留品牌原始比例、不拉伸**；SAKURA 維持既有 `170/180/210px` 寬度。
