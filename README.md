...
- **Browser checks**：1512、1024、390 三種寬度均無水平溢出、素材 natural size 正確、無 console error。三個 logo 的可見高度已統一：1512px 均為 `34px`（SVago `121.73px` 寬、TEKA `115.59px` 寬、SAKURA `209.66px` 寬），390px 均為 `28px`（寬度依原比例自動計算）；其新主圖分別在 1512px 為 `430×310`、390px 為 `340×250`。桌機／手機均確認只渲染這三張品牌卡。三張主圖均以 `object-cover` 正確填滿卡片。
...
- **品牌標題素材**：第 01 卡使用者提供的 `public/services/svago-logo-white.png`（SVago，`111×31`）與 `public/services/svago-product.png`（`1300×945`）；第 02 卡使用 `public/services/teka-logo-white.svg`（TEKA 向量 logo，viewBox `170×50`）與 `public/services/teka-product.png`（`977×945`）；第 03 卡為 SAKURA logo 與抽油煙機主圖。三張來源皆為白色透明圖，因此在白底 caption 中統一轉為原 h3 的深色。三個 logo 統一使用 mobile `28px`／tablet `31px`／desktop `34px` 的固定可見高度和 `width:auto`，**同尺寸且保留品牌原始比例、不拉伸**。
