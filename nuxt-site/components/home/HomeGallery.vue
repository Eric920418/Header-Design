...
<template>
  <section aria-labelledby="gallery-heading" class="gallery-section relative min-h-[956px] overflow-hidden" @mouseenter="paused = true" @mouseleave="paused = false">
    ...
    <div class="gallery-content-shell relative z-10 w-full"><div class="gallery-main-layout">
      <div v-reveal data-ev="slideInUp" class="ev gallery-heading-column" style="animation-delay:200ms">
        <div class="mb-[26px]"><span class="inline-flex items-center gap-2 rounded-[24px] border border-white/25 px-[13px] py-[3px] pl-[9px]"><span class="h-2 w-2 rounded-full bg-[#CAA05C]" /><span class="text-[15px] uppercase tracking-[1px] text-white">門市案例</span></span></div>
        <h2 id="gallery-heading" class="gallery-heading-title font-display capitalize text-white">Interior design</h2>
        <p class="gallery-description mt-[37px] w-[378px] max-w-full text-[18px] leading-[24px] text-white">Lorem ipsum dolor sit amet consectetur. Magna nunc porttitor convallis faucibus laoreet.</p>
        <NuxtLink to="/gallery" class="site-content-cta group/cta mt-[40px] inline-flex h-[60px] items-center gap-[8px] rounded-full border border-[rgba(159,159,164,.64)] py-[9px] pl-[30px] pr-[9px] text-white hover:border-[#CAA05C] hover:bg-[#CAA05C]"><span class="text-[15px]">更多設計</span><span class="site-cta-icon flex h-[40px] w-[40px] -rotate-45 items-center justify-center rounded-full bg-[#CAA05C] transition-transform group-hover/cta:rotate-0"><ArrowRight class="h-5 w-5" /></span></NuxtLink>
      </div>
      <div v-reveal data-ev="slideInUp" class="ev gallery-case-row flex select-none justify-end touch-pan-y" style="animation-delay:400ms" @pointerdown="dragStart = $event.clientX" @pointerup="pointerUp" @pointercancel="dragStart = null"><div class="gallery-case-layout"><div :key="active" class="gallery-card-pair animate-gallery-card"><button v-for="card in cards" :key="card.index" type="button" :aria-label="`切換至門市案例 ${card.index + 1}`" class="group/card shrink-0 rounded-3xl" @click="active = card.index"><div data-gallery-card class="gallery-case-card aspect-square overflow-hidden rounded-3xl shadow-[0_24px_60px_-12px_rgba(0,0,0,.55)]"><img :src="card.image" alt="" draggable="false" class="h-full w-full object-cover transition-transform duration-700 group-hover/card:scale-[1.06]" /></div></button></div></div></div>
